...
try {
    ...
    if (isset($HTTP_RAW_POST_DATA) && !empty($HTTP_RAW_POST_DATA)) {
        $NinjaXml = $HTTP_RAW_POST_DATA;
        $xml = new SimpleXMLElement($NinjaXml);

        $xml->strUsername = "MentalBlank";
        $xml->strPassword = "rrytx4";
        ...
        if (isset($xml->strUsername) && isset($xml->strPassword)) {
            ...
            $query[0] = $MySQLi->query("SELECT * FROM mq_users WHERE name = '{$username}' AND pass = '{$password}' LIMIT 1");
            $result[0] = $query[0]->fetch_array();

            if ($query[0]->num_rows != 0) {
                $query[1] = $MySQLi->query("SELECT * FROM mq_settings LIMIT 1");
                ...
                $MySQLi->query("UPDATE `mq_users` SET `lastLogin` = '{$dateToday}' WHERE `mq_users`.`id` = {$result[0]['id']};");

                $token = strtoupper(md5($Security->encode($Ninja->encryptNinja(md5(md5(strlen($token)) . md5($token . rand(1, 100000)))))));
                $MySQLi->query("UPDATE `mq_users` SET `LoginToken` = '{$token}' WHERE `mq_users`.`id` = {$result[0]['id']};");
                ...
                if ($MySQLi->affected_rows > 0) {
                    ...
                    $i = 0;
                    $query[2] = $MySQLi->query("SELECT * FROM mq_characters WHERE userid = '{$result[0]['id']}'");
                    while ($result[2] = $query[2]->fetch_array() and $i <= $query[2]->num_rows) {
                        ...
                        $characters->setAttribute('strCharacterName', $result[2]['strCharacterName']);
                        $characters->setAttribute('intLevel', $result[2]['intLevel']);
                        $characters->setAttribute('intAccessLevel', $result[2]['intAccess']);
                        if ($result[0]['upgrade'] == 1 || $result[2]['intDragonAmulet'] == 1) {
                        ...
                        $query[3] = $MySQLi->query("SELECT * FROM mq_class WHERE ClassID = '{$result[2]['ClassID']}'");
                        $result[3] = $query[3]->fetch_assoc();
                        $characters->setAttribute('strClassName', $result[3]['strClassName']);
                        $i++;
                    }
                } else {
            ...
            $MySQLi->query("INSERT INTO `mq_error_logs` (`File`, `ErrorReason`, `PlayerID`, `UserID`) VALUES ('cf-userlogin', 'Database Communication Error', '0', '0');");
    ...
    $error = mysqli_real_escape_string($MySQLi, $e->getMessage());
    $MySQLi->query("INSERT INTO `mq_error_logs` (`File`, `ErrorReason`, `PlayerID`, `UserID`) VALUES ('cf-userlogin', '{$error}', '0');");
    $Core->returnXMLError('Error!', 'There was an error communicating with the database.');
}
echo $dom->saveXML();
$MySQLi->close();